...
def get_items(page):
    json_path = 'items/page_%d.json' % page
...
def get_items2(page):
    ...
    if os.path.exists(json_path):
        return read_data(json_path)
    url = 'https://www.snpedia.com/index.php?title=Special:Ask&offset=%d' % page
    url += '&limit=500&q=%5B%5BCategory%3AIs+a+snp%5D%5D&p=mainlabel%3D%2Fformat%3Dtable&po=%3FMax+Magnitude%0A%3FSummary%0A&sort=Max+Magnitude&order=desc'
    items, data1, len_trs = get_table_datas(url, 0, base='')

    if len(items) > 0:
        write_data(items, json_path)
        # write_data(items, 'items/page_%d.xlsx' % page)
    return items
...
def main2():
    ...
    for i in range(total):
        ...
        print msg, get_time(), items.index(items1)
    print len(items2)
    write_data(items2, 'results/snpedia2.csv')
    write_data(items2, 'results/snpedia2.json')
